  ...
  */
  ...
  if (precision == DataType::kINT8) {
    ...
    for (int i = 0; i < weight.count; ++i) {
      float local_abs = std::abs(reinterpret_cast<const float*>(weight_host.data())[i]);
    ...
    for (int i = 0; i < weight.count; ++i) {
      scale_buffer[i] = static_cast<int8_t>(
          round_scale(reinterpret_cast<const float*>(weight_host.data())[i] / weight_scale_));
    }
...
IPluginV2DynamicExt* SpmmPluginDynamic::clone() const noexcept {
  try {
    auto* p = new SpmmPluginDynamic(
        layer_name_, precision_, out_dim_, k_, weight_compressed_,
        compressed_size_, bias_, is_configured_, m_max_, optim_alg_, activation_);
...
size_t SpmmPluginDynamic::getSerializationSize() const noexcept {
  return compressed_size_ + (has_bias_ ? sizeof(float) * out_dim_ : 0) +
         ...
         sizeof(optim_alg_) + sizeof(weight_scale_) + sizeof(compressed_size_) +
         sizeof(has_bias_) + sizeof(activation_);
}
