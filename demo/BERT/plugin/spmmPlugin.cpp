...
namespace {
const char* SPMM_VERSION{"1"};
const char* SPMM_NAME{"CustomSpmmPluginDynamic"};
}
...
inline int getElementSize(DataType type) {
  switch (type) {
    ...
    default: assert(false && "getElementSize only supports [FLOAT|HALF|INT8]");
  }
}
...
inline cudaDataType_t convertTrtType(DataType type) {
  switch (type) {
    ...
    case DataType::kINT8: return CUDA_R_8I;
    default: assert(false && "convertTrtType only supports [FLOAT|HALF|INT8]");
  }
}

inline void deserialize_value_size(void const** buffer, size_t* buffer_size, void* value, size_t value_size) {
...
inline void convertAndCopy(const Weights& src, DataType type, void* dest) {
  assert((src.type == DataType::kFLOAT ||
         src.type == DataType::kHALF) &&
...
void SpmmPluginDynamic::cusparseLtContext::init(
  ...
  */
  assert(!is_initialized && "Descriptor should be destroyed before calling create");
  constexpr int alignment = 16;
  cusparseComputeType compute_type;
  switch (type) {
    case CUDA_R_32F: compute_type = CUSPARSE_COMPUTE_TF32; break;
    case CUDA_R_16F: compute_type = CUSPARSE_COMPUTE_16F; break;
    case CUDA_R_8I: compute_type = CUSPARSE_COMPUTE_32I; break;
    default: assert(false && "cusparLtContext only supports data type [CUDA_R_32F|CUDA_R_16F|CUDA_R_8I]");
  }
  ...
  if (activation == SpmmPluginDynamic::Activation::kRelu) {
    ...
    CHECK_CUSPARSE(cusparseLtMatmulDescSetAttribute(
        ...
        sizeof(true_value)))
    CHECK_CUSPARSE(cusparseLtMatmulDescSetAttribute(
        &handle, &matmul, CUSPARSELT_MATMUL_ACTIVATION_GELU_SCALING, &gelu_scale,
        sizeof(gelu_scale)))
  } else {
...
void SpmmPluginDynamic::cusparseLtContext::setAlgo(int alg) {
  // Plan needs alg id, so if the alg id is changed, we need to reinitialize a new plan
  assert(is_initialized && "Descriptor should be initialized before setting algorithm");
  CHECK_CUSPARSE(cusparseLtMatmulAlgSetAttribute(
      &handle, &alg_sel, CUSPARSELT_MATMUL_ALG_CONFIG_ID, &alg, sizeof(alg)))
  CHECK_CUSPARSE(
      cusparseLtMatmulGetWorkspace(&handle, &alg_sel, &workspace_size))
...
void SpmmPluginDynamic::cusparseLtContext::compressMatB(
  ...
  CHECK_CUDA(cudaMalloc((void**)dest, *compressed_size))
...
SpmmPluginDynamic::SpmmPluginDynamic(const std::string& layer_name,
  ...
  */
  ...
  CHECK_CUDA(cudaMalloc((void**)&weight_dev, precision_size_ * out_dim_ * k_))
  if (precision == DataType::kINT8) {
    weight_scale_ = new float[out_dim_];
    for (int i = 0; i < out_dim_; ++i) {
      float max_weight{0.0f};
      for (int j = 0; j < k_; ++j) {
        float local_abs = std::abs(reinterpret_cast<const float*>(weight_host.data())[i * k_ + j]);
    ...
    for (int i = 0; i < weight.count; ++i) {
      scale_buffer[i] = static_cast<int8_t>(
          round_scale(reinterpret_cast<const float*>(weight_host.data())[i] / static_cast<float*>(weight_scale_)[i / k_]));
    }
    CHECK_CUDA(cudaMemcpy(weight_dev, scale_buffer.data(),
                          precision_size_ * weight.count,
                          cudaMemcpyHostToDevice))
  } else {
    CHECK_CUDA(cudaMemcpy(weight_dev, weight_host.data(), precision_size_ * weight.count,
                          cudaMemcpyHostToDevice))
  }
  ...
  if (has_bias_) {
    ...
    assert(bias.type == DataType::kFLOAT && "SpmmPluginDynamic only supports FLOAT bias");
...
SpmmPluginDynamic::SpmmPluginDynamic(const std::string& layer_name,
  ...
  */
  ...
  CHECK_CUDA(cudaMalloc((void**)&weight_compressed_dev_, compressed_size))
  CHECK_CUDA(cudaMemcpy(weight_compressed_dev_, weight_compressed_,
                        compressed_size, cudaMemcpyHostToDevice))
  
  if (precision_ == DataType::kINT8) {
    weight_scale_ = new float[out_dim_];
    std::copy_n(static_cast<const char*>(weight_scale), sizeof(float) * out_dim_, static_cast<char*>(weight_scale_));
    if (is_configured_) {
      CHECK_CUDA(cudaMalloc((void**)&weight_scale_dev_, sizeof(float) * out_dim_))
      CHECK_CUDA(cudaMemcpy(weight_scale_dev_, weight_scale_, sizeof(float) * out_dim_,
                            cudaMemcpyHostToDevice))
    }
  }
  ...
  if (has_bias_) {
    ...
    if (is_configured_) {
      CHECK_CUDA(cudaMalloc((void**)&bias_dev_, sizeof(float) * out_dim_))
  ...
  if (is_configured_) {
    ...
    spmm_context_.init(m_max_, out_dim_, k_, dataType, bias_dev_,
                         activation_, gelu_scale_);
...
SpmmPluginDynamic::SpmmPluginDynamic(const std::string name, const void* data,
  ...
  CHECK_CUDA(cudaMalloc((void**)&weight_compressed_dev_, compressed_size_))
  CHECK_CUDA(cudaMemcpy(weight_compressed_dev_, weight_compressed_,
  ...
  if (precision_ == DataType::kINT8) {
    weight_scale_ = new float[out_dim_];
    deserialize_value_size(&data, &length, weight_scale_, sizeof(float) * out_dim_);
    CHECK_CUDA(cudaMalloc((void**)&weight_scale_dev_, sizeof(float) * out_dim_))
    CHECK_CUDA(cudaMemcpy(weight_scale_dev_, weight_scale_, sizeof(float) * out_dim_,
                          cudaMemcpyHostToDevice))
  }
...
IPluginV2DynamicExt* SpmmPluginDynamic::clone() const noexcept {
  try {
    auto* p = new SpmmPluginDynamic(
        layer_name_, precision_, out_dim_, k_, weight_scale_, weight_compressed_,
        compressed_size_, bias_, is_configured_, m_max_, optim_alg_, activation_, gelu_scale_);
...
void SpmmPluginDynamic::configurePlugin(const DynamicPluginTensorDesc* inputs,
  ...
  */
  try {
    ...
    assert(inDims0.nbDims == 4);
    assert(k_ == inDims0.d[1]);
    assert(inDims0.d[2] == 1);
    ...
    if (precision_ == DataType::kINT8) {
      ...
      for (int i = 0; i < out_dim_; ++i) {
        static_cast<float*>(weight_scale_)[i] = static_cast<const float*>(weight_scale_)[i] * scale;
      }
      CHECK_CUDA(cudaMalloc((void**)&weight_scale_dev_, sizeof(float) * out_dim_))
      CHECK_CUDA(cudaMemcpy(weight_scale_dev_, weight_scale_, sizeof(float) * out_dim_,
                              cudaMemcpyHostToDevice))
    ...
    if (has_bias_) {
      if (inputs->desc.type == DataType::kINT8 && activation_ == Activation::kNone) {
        for (int i = 0; i < out_dim_; ++i) {
          static_cast<float*>(bias_)[i] = static_cast<const float*>(bias_)[i] / outputs->desc.scale;
        }
      }
      CHECK_CUDA(cudaMalloc((void**)&bias_dev_, sizeof(float) * out_dim_))
      CHECK_CUDA(cudaMemcpy(bias_dev_, bias_, sizeof(float) * out_dim_,
    ...
    float alpha{1.0f};
    float beta{0.0f};
    CHECK_CUDA(cudaMalloc((void**)&dA, m_max_ * k_ * sizeof(dataType)));
    CHECK_CUDA(cudaMalloc((void**)&dC, m_max_ * out_dim_ * sizeof(dataType)));
    CHECK_CUDA(cudaMalloc((void**)&d_workspace, spmm_context_.workspace_size));
    void* alpha_vec;
    if (precision_ == DataType::kINT8) {
...
int SpmmPluginDynamic::initialize() noexcept {
  return 0;
}
...
size_t SpmmPluginDynamic::getSerializationSize() const noexcept {
  return compressed_size_ + (has_bias_ ? sizeof(float) * out_dim_ : 0) +
         sizeof(precision_) + sizeof(precision_size_) + sizeof(element_size_) + sizeof(out_dim_) +
         sizeof(k_) + sizeof(m_max_) + sizeof(is_configured_) +
         sizeof(optim_alg_) + (precision_ == DataType::kINT8 ? sizeof(float) * out_dim_ : 0) + sizeof(compressed_size_) +
         sizeof(has_bias_) + sizeof(activation_) + sizeof(gelu_scale_);
}
...
void SpmmPluginDynamic::serialize(void* buffer) const noexcept {
  ...
  if (precision_ == DataType::kINT8) {
    ...
    std::copy_n(static_cast<const char*>(weight_scale_), out_dim_ * sizeof(float), d);
  }
}

void SpmmPluginDynamic::destroy() noexcept {
  delete[](char*) weight_compressed_;
