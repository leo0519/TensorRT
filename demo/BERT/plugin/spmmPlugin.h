  ...
  {                                                                           \
    ...
    if (s_ != cudaSuccess) {                                                  \
      printf("CUDA API failed at line %d,%s with error: %s (%d)\n", __LINE__, \
             __FILE__, cudaGetErrorString(s_), s_);                           \
      assert(false);                                                          \
    }                                                                         \
  }
...
class SpmmPluginDynamic : public nvinfer1::IPluginV2DynamicExt {
 ...
 private:
  ...
  size_t element_size_;          // size of weight (float if INT8 or FLOAT; half if HALF)
  int out_dim_;
  int k_;
  int m_max_;
