...
def insert_quantize(network, tensor, scale):
    scale_layer = network.add_constant((1, ), trt.Weights(np.ascontiguousarray([scale / 127.0], dtype=np.float32)))
    quantize_layer = network.add_quantize(tensor, scale_layer.get_output(0))
    quantize_layer.axis = -1
    return quantize_layer

def insert_dequantize(network, tensor, scale):
    scale_layer = network.add_constant((1, ), trt.Weights(np.ascontiguousarray([scale / 127.0], dtype=np.float32)))
    dequantize_layer = network.add_dequantize(tensor, scale_layer.get_output(0))
    dequantize_layer.axis = -1
    return dequantize_layer

def insert_quantize_dequantize(network, tensor, scale, insert_quantize=True, insert_dequantize=True):
    scale_tensor = network.add_constant((1, ), trt.Weights(np.ascontiguousarray([scale / 127.0], dtype=np.float32))).get_output(0)
    if insert_quantize:
        layer = network.add_quantize(tensor, scale_tensor)
        layer.axis = -1
        tensor = layer.get_output(0)
    if insert_dequantize:
        layer = network.add_dequantize(tensor, scale_tensor)
        layer.axis = -1
    return layer

def insert_quantize_dequantize_channel_wise(network, out_channel, weight):
    weight_numpy = np.copy(weight.numpy()).reshape(out_channel, -1, 1, 1).astype(np.float32)
    weight_amax = np.abs(weight_numpy).max(axis=(1, 2, 3)) / 127.0
    weight_const_layer = network.add_constant(weight_numpy.shape, weight)
    weight_scale_layer = network.add_constant(weight_amax.shape, trt.Weights(weight_amax))
    weight_quantize_layer = network.add_quantize(weight_const_layer.get_output(0), weight_scale_layer.get_output(0))
    weight_dequantize_layer = network.add_dequantize(weight_quantize_layer.get_output(0), weight_scale_layer.get_output(0))
    weight_quantize_layer.axis = 0
    weight_dequantize_layer.axis = 0
    return weight_dequantize_layer
...
def attention_layer_opt(prefix, config, init_dict, network, input_tensor, mask_idx, cu_seqlens, max_seqlen):
    ...
    # FC_attention
    if config.use_int8:
        mult_all = network.add_convolution_nd(input_tensor, 3 * hidden_size, (1, 1), trt.Weights(), Ball)
    else:
        mult_all = network.add_fully_connected(input_tensor, 3 * hidden_size, trt.Weights(), Ball)
    Wall_dequantize_layer = insert_quantize_dequantize_channel_wise(network, 3 * hidden_size, Wall)
    mult_all.set_input(1, Wall_dequantize_layer.get_output(0))
    ...
    if config.use_qat:
        ...
        qkv2ctx = insert_quantize_dequantize(network, qkv2ctx.get_output(0), dr_ctx)
...
def transformer_layer_opt(prefix, config, init_dict, network, input_tensor, residual, mask_idx, cu_seqlens, max_seqlen):
    ...
    """
    hidden_size = config.hidden_size
    is_first_trans = prefix.startswith('l0')
    ...
    if config.use_qat:
        ...
        if is_first_trans:
            skln_input_tensor = insert_quantize(network, input_tensor, dr_input).get_output(0)
            attention_layer_opt_input_tensor = insert_dequantize(network, skln_input_tensor, dr_input).get_output(0)
        else:
            skln_input_tensor = input_tensor
            attention_layer_opt_input_tensor = insert_dequantize(network, skln_input_tensor, dr_input).get_output(0)

    context_transposed = attention_layer_opt(prefix + "attention_", config, init_dict, network, attention_layer_opt_input_tensor, mask_idx, cu_seqlens, max_seqlen)
    ...
    if config.use_int8:
        attention_out_fc = network.add_convolution_nd(attention_heads, hidden_size, (1, 1), trt.Weights(), B_aout)
    else:
        attention_out_fc = network.add_fully_connected(attention_heads, hidden_size, trt.Weights(), B_aout)
    W_aout_dequantize_layer = insert_quantize_dequantize_channel_wise(network, hidden_size, W_aout)
    attention_out_fc.set_input(1, W_aout_dequantize_layer.get_output(0))
    if config.use_int8 and config.use_qat:
        dr_fc_aout = init_dict[prefix + 'attention_output_add_local_input_quantizer_amax']
        attention_out_fc = insert_quantize(network, attention_out_fc.get_output(0), dr_fc_aout)

    if config.use_megatron:
        dr_skln1_res_in = init_dict[prefix + "attention_output_add_residual_input_quantizer_amax"]
        skip = residual
    else:
        skip = skln_input_tensor
    ...
    if config.use_qat:
        ...
        mid_dense_input_tensor = insert_dequantize(network, attention_ln, dr_skln1).get_output(0)

    # FC1 + GELU
    B_mid = init_dict[prefix + B_MID]
    W_mid = init_dict[prefix + W_MID]
    if config.use_int8:
        mid_dense = network.add_convolution_nd(mid_dense_input_tensor, config.intermediate_size, (1, 1), trt.Weights(), B_mid)
    else:
        mid_dense = network.add_fully_connected(mid_dense_input_tensor, config.intermediate_size, trt.Weights(), B_mid)
    W_mid_dequantize_layer = insert_quantize_dequantize_channel_wise(network, config.intermediate_size, W_mid)
    mid_dense.set_input(1, W_mid_dequantize_layer.get_output(0))
    ...
    if config.use_int8:
        if config.use_qat:
            ...
            gelu_layer = insert_quantize_dequantize(network, intermediate_act, dr_gelu)
            intermediate_act = gelu_layer.get_output(0)
    ...
    if config.use_int8:
        out_dense = network.add_convolution_nd(intermediate_act, hidden_size, (1, 1), trt.Weights(), B_lout)
    else:
        out_dense = network.add_fully_connected(intermediate_act, hidden_size, trt.Weights(), B_lout)
    W_lout_dequantize_layer = insert_quantize_dequantize_channel_wise(network, hidden_size, W_lout)
    out_dense.set_input(1, W_lout_dequantize_layer.get_output(0))
    if config.use_int8 and config.use_qat:
        dr_fc_out = init_dict[prefix + 'output_add_local_input_quantizer_amax']
        out_dense = insert_quantize(network, out_dense.get_output(0), dr_fc_out)
...
def bert_model(config, init_dict, network, input_tensor, residual, mask_idx, cu_seqlens, max_seqlen):
    ...
    if config.use_qat:
        ...
        prev_input = insert_dequantize(network, prev_input, dr_out).get_output(0)
...
def squad_output(prefix, config, init_dict, network, input_tensor):
    ...
    if config.use_int8:
        dense = network.add_convolution_nd(input_tensor, 2, (1, 1), trt.Weights(), B_out)
    else:
        dense = network.add_fully_connected(input_tensor, 2, trt.Weights(), B_out)
    W_out_dequantize_layer = insert_quantize_dequantize_channel_wise(network, 2, W_out)
    dense.set_input(1, W_out_dequantize_layer.get_output(0))
